...
export const TableHeader = React.forwardRef<HTMLTableSectionElement, TableHeaderProps>(
  ({ className = '', children, ...props }, ref) => (
    <thead ref={ref} className={`flx-table__header bg-muted/50 border-b border-solid border-border ${className}`} {...props}>
      {children}
    </thead>
  )
);
...
export const TableBody = React.forwardRef<HTMLTableSectionElement, TableBodyProps>(
  ({ className = '', children, ...props }, ref) => (
    <tbody ref={ref} className={`flx-table__body ${className}`} {...props}>
      {children}
    </tbody>
  )
);
...
export const TableRow = React.forwardRef<HTMLTableRowElement, TableRowProps>(
  ({ selected = false, className = '', children, ...props }, ref) => {
    const classes = ['flx-table__row transition-colors', selected ? 'flx-table__row--selected bg-primary/5' : '', className]
      .filter(Boolean)
      .join(' ');
    return (
